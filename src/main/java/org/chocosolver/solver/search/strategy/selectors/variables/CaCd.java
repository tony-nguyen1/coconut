 ...
 * @since 12/06/20
 */
public class CaCd<V extends Variable> extends DomOverWDeg<V> {
     ...
     */
    public CaCd(V[] variables, long seed) {
     ...
     */
    public CaCd(V[] variables, long seed, int flushThs) {
    ...
    void increase(Propagator<?> prop, Element elt, double[] ws) {
        ...
        for (int i = 0; i < prop.getNbVars(); i++) {
            ...
            inc[0] = 1.0 / (futvars * ( 1 + ivar.getDomainSize()));
            //avant :
